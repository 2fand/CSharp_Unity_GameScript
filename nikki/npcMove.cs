...
public class npcMove : MonoBehaviour
{
    ...
    wasd getwasd()
    {
        ...
        if (move.Catch == moveMode || move.escape == moveMode)
        {
            ...
            Debug.Log(compare[0].ToString() + " " + compare[1].ToString() + " " + compare[2].ToString() + " " + compare[3].ToString());
        ...
        switch (moveMode){
            case move.Catch:
                for (i = 0; i < 4; i++)
                {
                    ...
                    {
                        case wasd.w:
                            if (m.verticalIsCycle ? (' ' == m.wmap[npc.x, npc.y - 1 >= 0 ? npc.y - 1 : m.y - 1]) : (0 != npc.y && ' ' == m.wmap[npc.x, npc.y - 1]))
                            {
                                goto end;
                            }
                            break;
                        case wasd.a:
                            if (m.horizontalIsCycle ? (' ' == m.wmap[npc.x - 1 >= 0 ? npc.x - 1 : m.x - 1, npc.y]) : (0 != npc.x && ' ' == m.wmap[npc.x - 1, npc.y]))
                            {
                                goto end;
                            }
                            break;
                        case wasd.s:
                            if (m.verticalIsCycle ? (' ' == m.wmap[npc.x, (npc.y + 1) % m.y]) : (npc.y != m.y - 1 && ' ' == m.wmap[npc.x, npc.y + 1])) {
                                goto end;
                            }
                            break;
                        default:
                            if (m.horizontalIsCycle ? (' ' == m.wmap[(npc.x + 1) % m.x, npc.y]) : (npc.x != m.x - 1 && ' ' == m.wmap[npc.x + 1, npc.y])) {
                                goto end;
                            }
                            break;
                    }
