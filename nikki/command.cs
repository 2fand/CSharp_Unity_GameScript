...
public abstract class command
{
    ...
    public abstract bool setValue(string value, int valueNumber, trigger _trigger);using System;
using System.Collections;
using System.Collections.Generic;
using System.Text.RegularExpressions;
using Unity.VisualScripting;
...
public class _goto : command
{
    ...
    public override bool setValue(string value, int valueNumber, _runCommands commandsValues)
    {
        if (!commandsValues.labels.ContainsKey(value))
        {
            Debug.Log("标签错误：标签不存在");
            return false;
        }
        commandsValues.commandI = (int)commandsValues.labels[value];
        return false;
    }
    public override bool execute()
    {
...
public class set : command
{
    ...
    public override bool setValue(string value, int valueNumber, _runCommands commandsValues)
    {
        ...
        {
            jsonValue.valueClass status = jsonValue.valueClass.value;
            if ('[' == value[0])
            {
                status = jsonValue.valueClass.array;
            }
            else if ('{' == value[0]) { 
                status = jsonValue.valueClass.@object;
            }
            switch (status)
            {
                case jsonValue.valueClass.value:
                    isJsonObject = Regex.Match(value, "^([+-]?([0-9]+(\\.([+-]?[0-9]+)?)?|\\.[0-9]+)([Ee][+-]?[0-9]+)?)|true|false|null|\"\".+?\"\"$").Success;
                    jsonObject = value;
                    commandsValues.vars[_varName] = new jsonValue(jsonObject);
                    break;
                case jsonValue.valueClass.array:
    ...
    public override bool execute()
    {
        if (!Regex.Match(_varName, "^[_a-zA-z][_0-9a-zA-z]*$").Success && isJsonObject)
        {
            return false;
        }
        return true;
