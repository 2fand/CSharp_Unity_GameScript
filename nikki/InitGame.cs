...
public class InitGame : MonoBehaviour
{
    ...
    void Awake()
    {
        if (!isInit)
        {
            ...
            effectItem.effectCloseup = new closeUp[closeUps.Length / 3];
            for (int i = 0; i < closeUps.Length / 3; i++)
            {
                effectItem.effectCloseup[i] = new closeUp(closeUps[i * 3], closeUps[i * 3 + 1], closeUps[i * 3 + 2]);
            }
