...
public class zComponentUI : Editor
{
    ...
    {
        z.haveFront = !EditorGUILayout.Toggle("是否无视方向", !z.haveFront);
        z.mod = (z.mode)EditorGUILayout.EnumPopup("交互模式", z.mod);
        switch (z.mod) {
            case z.mode.tele:
                z.teleWaitTime = EditorGUILayout.FloatField("传送时等待时间", z.teleWaitTime);
                z.exitMode = (change.transitionMode)EditorGUILayout.EnumPopup("转场退出模式", z.exitMode);
                z.exitModeTime = EditorGUILayout.FloatField("转场退出时间", z.exitModeTime);
                z.enterMode = (change.transitionMode)EditorGUILayout.EnumPopup("转场进入模式", z.enterMode);
                z.enterModeTime = EditorGUILayout.FloatField("转场进入时间", z.enterModeTime);
