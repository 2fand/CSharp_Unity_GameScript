...
public abstract class item
{
    ...
    {
        you.items.Add(item);
        if (item is effectItem && !you.effecthaves[(int)((effectItem)item).effect])
        {
            you.effecthaves[(int)((effectItem)item).effect] = true;
            you.effectNum++;
        }
        if (!item.isHide)
        {
    ...
    public static void delItem(item item)
    {
        ...
        if (item is effectItem && you.effecthaves[(int)((effectItem)item).effect])
        {
            you.effecthaves[(int)((effectItem)item).effect] = false;
            you.effectNum--;
        }
        else if (item is effectItem && 0 == ((effectItem)item).effect) {
            return;
        }
